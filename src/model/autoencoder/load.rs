...
fn load_padded_conv2d<B: Backend>(
    ...
    let mut padded_conv: PaddedConv2d<B> = PaddedConv2dConfig::new(channels, kernel_size, padding)
        ...
        .init(device);
    let padding_actual =
        PaddingConfig2d::Explicit(padded_conv.padding_actual[0], padded_conv.padding_actual[1],padded_conv.padding_actual[0], padded_conv.padding_actual[1]);

    conv.padding =  padding_actual;
