...
pub fn numpy_to_tensor<B: Backend + burn::tensor::backend::BackendTypes, const D: usize>(
    ...
    let data: Vec<<B as burn::tensor::backend::BackendTypes>::FloatElem> = v[D..].into_iter().map(|e| e.elem()).collect();
...
pub fn load_conv2d<B: Backend>(
    ...
    let padding = nn::PaddingConfig2d::Explicit(padding[0], padding[1], padding[0], padding[1]);
    ...
    conv2d.padding = padding.clone();
        
    Ok(conv2d)
}

pub fn tensor_to_array_2<B: Backend + burn::tensor::backend::BackendTypes>(x: Tensor<B, 1>) -> [usize; 2] {
    let vec: Vec<<B as burn::tensor::backend::BackendTypes>::FloatElem> = x.into_data().to_vec().unwrap();
    assert!(vec.len() == 2, "Tensor length must be 2.");
    [vec[0].to_usize(), vec[1].to_usize()]
}

pub fn tensor_to_array<const N: usize, B: Backend + burn::tensor::backend::BackendTypes>(x: Tensor<B, 1>) -> [usize; N] {
    let vec: Vec<<B as burn::tensor::backend::BackendTypes>::FloatElem> = x.into_data().to_vec().unwrap();
